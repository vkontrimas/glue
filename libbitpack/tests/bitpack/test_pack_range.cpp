...
TEST(BitpackRangeDeathTests, WhenMoreThan32BitsNeeded_Dies) {
  ...
  EXPECT_DEATH(pack_range_checked(packer, value, 0ull, 2131241257812681275ull),
               "Assertion.*");
  EXPECT_DEATH(
      pack_range_checked(unpacker, value, 0ull, 11241247865178651237ull),
      "Assertion.*");

  i64 signed_value;
  EXPECT_DEATH(pack_range_checked(packer, signed_value, -189751571892375ll,
                                  123512512351235ll),
               "Assertion.*");
  EXPECT_DEATH(pack_range_checked(unpacker, signed_value, -1247829752892372ll,
                                  1512356782627823ll),
               "Assertion.*");
}
...
TEST(BitpackRangeDeathTests, WhenBeforeBegin_Dies) {
  ...
  EXPECT_DEATH(pack_range_checked(packer, value, 0, 10), "Assertion.*");
}
...
TEST(BitpackRangeTests, UnpackValues) {
  ...
  u8 value_byte{0};
  pack_range_checked(unpacker, value_byte, 200, 240);
  EXPECT_EQ(value_byte, 211);

  u16 value_short{0};
  pack_range_checked(unpacker, value_short, 5000, 6024);
  EXPECT_EQ(value_short, 5187);

  i32 value_int{0};
  pack_range_checked(unpacker, value_int, -2000, 2000);
  EXPECT_EQ(value_int, -1519);

  i64 value_long{0};
  pack_range_checked(unpacker, value_long, -1000000000000000015,
                     -1000000000000000000);
